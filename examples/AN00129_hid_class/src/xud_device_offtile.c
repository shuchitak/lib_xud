...
XUD_Result_t SetEndpointHalt_offtile(chanend_t chan_ep0_proxy, unsigned epNum, unsigned halt)
{
    /* Inspect for IN bit */
    if(epNum & 0x80)
    {
    ...
    USB_SetupPacket_t *sp, XUD_BusSpeed_t usbBusSpeed)
{
            ...
            switch(sp->bRequest)
            {
                ...
                case USB_SET_ADDRESS:
                    ...
                    if((sp->wValue < 128) && (sp->wIndex == 0) && (sp->wLength == 0))
                    {
                        ...
                        if((result = offtile_set_req_status(chan_ep0_proxy)) != XUD_RES_OKAY)
                        {
                            return result;
                        }
                        ...
                        {
                            hwtimer_delay(t, 50000);
                        }
                        hwtimer_free(t);

                        /* Set the device address in XUD */
                        //XUD_HAL_SetDeviceAddress(sp.wValue);
                        offtile_hal_set_device_address(chan_ep0_proxy, sp->wValue);
                        return XUD_RES_OKAY;
                ...
                case USB_SET_CONFIGURATION:
                    ...
                    if((sp->wLength == 0) && (sp->wIndex == 0))
                    {
                        ...
                        for(unsigned i = 1; i < USB_MAX_NUM_EP_IN; i++)
                        {
                            ...
                            offtile_reset_ep_state_by_addr(chan_ep0_proxy, i | 0x80);
                        }

                        for(unsigned i = 1; i < USB_MAX_NUM_EP_OUT; i++)
                        {
                            //XUD_ResetEpStateByAddr(i);
                            offtile_reset_ep_state_by_addr(chan_ep0_proxy, i);
                        }
                        ...
                        return offtile_set_req_status(chan_ep0_proxy);
                    }
                    break;
        ...
        case USB_BMREQ_D2H_STANDARD_DEV:
            ...
            switch(sp->bRequest)
            {
                ...
                case USB_GET_STATUS:
                    ...
                    return offtile_do_get_request(chan_ep0_proxy, buffer, 2);
                ...
                case USB_GET_CONFIGURATION:
                    ...
                    if((sp->wValue == 0) && (sp->wIndex == 0) && (sp->wLength == 1))
                    {
                        ...
                        return offtile_do_get_request(chan_ep0_proxy, buffer, 1);
                    }
                    break;
                    ...
                    switch(sp->wValue & 0xff00)
                    {
                        ...
                        case (USB_DESCTYPE_DEVICE << 8):
                            ...
                            if((sp->wValue & 0xff) == 0)
                            {
                                if((usbBusSpeed == XUD_SPEED_FS) && (devDescLength_fs != 0))
                                {
                                    ...
                                    return offtile_do_get_request(chan_ep0_proxy, devDesc_fs, devDescLength_fs);
                                }
                                else if(devDescLength_hs != 0)
                                {
                                    ...
                                    return offtile_do_get_request(chan_ep0_proxy, devDesc_hs, devDescLength_hs);
                                }
                            }
                            break;
                        ...
                        case (USB_DESCTYPE_CONFIGURATION << 8):
                            ...
                            //if((sp.wValue & 0xff) == 0)
                            {
                                if((usbBusSpeed == XUD_SPEED_FS) && (cfgDescLength_fs != 0))
                                {
                                    ...
                                    return offtile_do_get_request(chan_ep0_proxy, cfgDesc_fs, cfgDescLength_fs);
                                }
                                else if(cfgDescLength_hs != 0)
                                {
                                    /* Do get request (send descriptor then 0 length status stage) */
                                    ...
                                    return offtile_do_get_request(chan_ep0_proxy, cfgDesc_hs, cfgDescLength_hs);
                                }
                            }
                            break;
                        ...
                        case (USB_DESCTYPE_DEVICE_QUALIFIER << 8):
                            ...
                            if((sp->wValue & 0xff) == 0)
                            {
                                ...
                                if((usbBusSpeed == XUD_SPEED_HS) && (devDescLength_fs != 0))
                                {
                                    ...
                                    return offtile_do_get_request(chan_ep0_proxy, devQualDesc, 10);
                                }
                                else if(devDescLength_hs != 0)
                                {
                        ...
                        case (USB_DESCTYPE_STRING << 8):
                            ...
                            if(stringID < strDescsLength)
                            {
                                ...
                                return offtile_do_get_request(chan_ep0_proxy, buffer, datalength + 2);
            ...
            switch(sp->bRequest)
            {
                ...
                case USB_SET_FEATURE:
                    ...
                    if(sp->wLength == 0)
                    {
                        ...
                        if(sp->wValue == USB_ENDPOINT_HALT)
                        {
                            ...
                            if(!SetEndpointHalt_offtile(chan_ep0_proxy, sp->wIndex, 1))
                            {
                                //return XUD_DoSetRequestStatus(ep_in);
                                return offtile_set_req_status(chan_ep0_proxy);
                            }
                ...
                case USB_CLEAR_FEATURE:
                    ...
                    if(sp->wLength == 0)
                    {
                        ...
                        {
                            /* Returns 0 on non-error */
                            if(!SetEndpointHalt_offtile(chan_ep0_proxy, sp->wIndex, 0))
                            {
                                //return XUD_DoSetRequestStatus(ep_in);
                                return offtile_set_req_status(chan_ep0_proxy);
                            }
    ...
    //XUD_SetStall(ep_out);
    //XUD_SetStall(ep_in);
    offtile_set_stall(chan_ep0_proxy);
    return XUD_RES_ERR;
}